...
/**
 * Configuration for receiving user activity events.
 *
...
@XSlf4j
public class ReceiveUserActivityEventsConfig {

  @Bean
  public Consumer<UserLoginEvent> userLoggedIn(@NotNull final LoggingEventBus bus) {
    return event -> {
      log.entry(event);
  ...
  @Bean
  public Consumer<UserLogoutEvent> userLoggedOut(@NotNull final LoggingEventBus bus) {
    return event -> {
      log.entry(event);
