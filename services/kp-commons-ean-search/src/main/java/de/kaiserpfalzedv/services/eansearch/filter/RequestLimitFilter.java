...
/**
 * RequestLimitFilter -- Reports the used credits and the remaining credits for the EAN search
 * domain.
 *
...
public class RequestLimitFilter implements ExchangeFilterFunction {
  ...
  private static final String API_REQUESTS_HANDLED_SINCE_START
      = "ean-search.credits.used-since-start";
  ...
  /**
   * Initializes the filter and registers the metrics.
   *
   */
  @PostConstruct
  ...
  /**
   * Unregisters the metrics when the application is shutting down.
   */
  @PreDestroy
  ...
  public void reset() {
    synchronized (this) {
      this.remaining = -1;
      log.info(
          "EAN Search credit reporter reset. Requests should now result in requests to the API.");
    }
  }
  ...
  public Mono<ClientResponse> filter(
    ...
    return log.exit(next.exchange(request).doOnNext(response -> {
      final String remainingRequestCount = response.headers().header(API_REMAINING_REQUEST_HEADER)
  ...
  private boolean lastRequestWasToday() {
    return this.lastRequest.isAfter(
        OffsetDateTime.now(ZoneOffset.UTC).toLocalDate().atStartOfDay(ZoneOffset.UTC)
                      .toOffsetDateTime());
  }
}
