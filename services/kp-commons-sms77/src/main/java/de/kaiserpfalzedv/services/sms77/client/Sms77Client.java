 ...
 *
 * <p>This is the client for accessing the API of the sms77 paid webservice. You need an Api-Key.
 * This client has a spring boot application.yaml included which will rely on certain environment
 * variables to be set. These are:</p>
 *
 * <dl>
 * <dt>SMS77_API_URL</dt>
 * <dd><em>(optional)</em>The URI for the SMS77.io domain. Normally there is no reason to give
 * another URI than {@literal https://gateway.sms77.io}. And that URI is the default when nothing
 * else is specified./</dd>
 * <dt>SMS77_API_KEY</dt>
 * <dd>The API key from sms77.io. For development you should generate a sandbox domain key to cut
 * costs - but your mileage may vary.</dd>
 * </dl>
 *
 * <p>For the time being the API does not throw any sms77 specific exceptions since the sms77 API
 * always returns HTTP 200. You have to check the return objects of the calls to find out if
 * there has something happened.</p>
 *
...
public interface Sms77Client {
   ...
   *
   * <p>This check only validates the number format and not if the number is used or even
   * active.</p>
   *
   * <p>This is a very ugly API call since the numbers need to be formated as single
   ...
   *
   * <p>This function only checks, if the number format is correct. There is no check
   * if the number is assigned or even activ.</p>
   *
