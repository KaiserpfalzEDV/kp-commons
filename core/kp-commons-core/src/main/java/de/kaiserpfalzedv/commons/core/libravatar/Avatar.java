...
public class Avatar implements de.kaiserpfalzedv.commons.api.libravatar.Avatar {
	...
	public byte[] download(final de.kaiserpfalzedv.commons.api.libravatar.AvatarOptions options) {
		try (InputStream is = new URL(this.buildUrl(options)).openStream()) {
	...
	public String buildUrl(final AvatarOptions options) {
		...
		StringBuilder sb = new StringBuilder(
				options.useHttps() ?
				  options.secureBaseUri()
				: options.baseUri()
		);
		
		sb.append(
				options.useSHA256() ?
				  DigestUtils.sha256Hex(this.email.toLowerCase().getBytes())
		...
		sb.append(
				options.defaultImage() != LibravatarDefaultImage.DEFAULT ?
				  "&d=" + options.defaultImage().getCode()
				: ""
		);
